var Silverlight, slPluginHTML = {};  // eslint-disable-line no-var
...
/**
 * Check silver is installed
 * @param { window.location.reload(false);} type
 */
        ...
        try {
            ...
            if ( plugin ) {
                if ( version === null ) {
                    ...
                    let actualVer = plugin.description;
                    if ( actualVer === '1.0.30226.2') { actualVer = '2.0.30226.2'; }
                    let actualVerArray =actualVer.split('.');
                    ...
                    let reqVerArray = version.split('.');
                    while ( reqVerArray.length > 4) {
                        reqVerArray.pop();
                    }

                    let requiredVersionPart;
                    let actualVersionPart;
                    let index = 0;
...
Silverlight.createObject = function(source, parentElement, id, properties, events, initParams, userContext) {
    let slPluginHelper = new Object();
    let slProperties = properties;
    let slEvents = events;
    ...
    if (Silverlight.isInstalled(slPluginHelper.version)) {
        ...
        for (let name in slEvents) {
            if ( slEvents[name]) {
                if ( name == 'onLoad' && typeof slEvents[name] == 'function' && slEvents[name].length != 1 ) {
                    var onLoadHandler = slEvents[name];
                    slEvents[name]=function(sender) { return onLoadHandler(document.getElementById(id), userContext, sender); };
                }
                let handlerName = Silverlight.__getHandlerName(slEvents[name]);
                if ( handlerName != null ) {
...
Silverlight.createObjectEx = function(params) {
    ...
    let html = Silverlight.createObject(parameters.source, parameters.parentElement, parameters.id, parameters.properties, parameters.events, parameters.initParams, parameters.context);
...
Silverlight.buildPromptHTML = function(slPluginHelper) {
    let slPluginHTML = '';
    let urlRoot = Silverlight.fwlinkRoot;
    let shortVer = slPluginHelper.version;
...
Silverlight.getSilverlight = function(version) {
    ...
    let shortVer = '';
    let reqVerArray = String(version).split('.');
    if (reqVerArray.length > 1) {
        let majorNum = parseInt(reqVerArray[0] );
...
Silverlight.__cleanup = function() {
    for (let i = Silverlight._silverlightCount - 1; i >= 0; i--) {
...
Silverlight.__getHandlerName = function(handler) {
    ...
    if ( typeof handler == 'string') {
        ...
        let count = Silverlight._silverlightCount++;
